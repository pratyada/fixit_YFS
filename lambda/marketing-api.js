...
async function handleEmailGenerate(event) {
  ...
  if (brand === 'personal') {
    ...
    system = `You are writing a SHORT, genuinely PERSONAL email as ${who}, in the first person ("I"), like a plain note to a friend — NOT a corporate newsletter. It can be any personal occasion the topic calls for: birthday wishes, a dinner or catch-up invitation, a thank-you, a heartfelt check-in. Warm, human, specific, a little excited, 90–170 words, in ${who}'s own voice. It must read like normal typed text: NO buttons, NO CTA, NO infographics, NO stat/research/details boxes, NO colored boxes of any kind, NO clinical jargon, NO headings, NO emoji. Crucially, this is NOT a business email — NO company or brand names, NO YourFormSux branding, NO discounts, offers, or promotions of any kind. ONLY a few short <p style="margin:0 0 14px">…</p> paragraphs of plain text. If you'd like them to reply or meet, just say so warmly in a sentence — never a button or offer. ${htmlRules}
Do not write a sign-off/signature; the template adds "Best regards, ${who}".`;
  } else if (brand === 'yfs') {
    system = `You are the voice of the YourFormSux TEAM — a Toronto fitness & movement community AND business (events, meetups, training, watch parties, offers). Write a friendly, energetic, BRANDED email that clearly comes from "the YourFormSux team" — use "we". The topic can be anything: a newsletter, an invitation, an announcement, or an occasion like birthday/holiday wishes. Warm and human, not clinical. 120–300 words. ${htmlRules}
Structure: open with a short paragraph to {{firstName}}; 1–3 short sections led by <h2 style="font-size:19px;color:#4E4E53;margin:22px 0 8px">…</h2> with <p style="margin:0 0 14px">…</p>, and where useful a <ul style="margin:0 0 14px;padding-left:20px;line-height:1.7">…</ul>.
Add a BUSINESS touch where it fits the topic — e.g. an invitation, a special offer, or a birthday/loyalty discount. When there is an event or an offer, include ONE box: <div style="background:#eef5f2;border-radius:12px;padding:16px 18px;margin:18px 0"><div style="font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#708E86;margin-bottom:6px">🎁 THE DETAILS</div><div style="font-size:14px;color:#44504c;line-height:1.6">WHAT / WHEN / WHERE, or the offer &amp; how to redeem</div></div> (use 📅 for an event, 🎁 for an offer).
${ctaBtn('Count me in')} Use CTA text matching the topic (e.g. "Count me in", "Save my spot", "Claim my offer").`;
  } else {
...
async function handleKioskChat(event) {
  ...
  try {
    ...
    if (roster.length) {
      ...
      rosterBlock = `\n\nKNOWN MEMBERS (the speech-to-text often mangles names — map what you hear to the CLOSEST member on this list, allowing for accents and phonetic errors, e.g. "Pratik"/"Prithak" → "Prateek", "Deepuk" → "Deepak"):\n${names}\n\nWhen the person says their name: silently pick the closest member from this list and greet them by that CORRECTED spelling, then confirm it — e.g. "Great to see you, Prateek — did I get that right?" — and STOP there. Ask NOTHING else in that reply (do not ask about the exercise yet) and wait for their yes/no. If they say it's wrong or say a different name, apologize briefly, re-match to the closest member, and confirm the new name. Only AFTER they confirm the name is right do you move on to asking which exercise. If two members are similarly close, ask a quick distinguisher (last initial). If nothing on the list is close, treat them as a new walk-in and just ask them to spell their first name. Never invent a member who isn't on the list.`;
    }
...
- Ask only ONE question per reply and then wait for the answer. NEVER bundle two steps into one reply — e.g. do not confirm their name and ask about the exercise in the same breath. Confirm the name first, wait for yes/no, and only move on once it's confirmed.
