...
import { Dumbbell, Camera, CheckCircle2, ChevronRight, Sparkles, ClipboardList } from 'lucide-react';
          ...
          <section key={regionKey} style={{ marginBottom: '26px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '12px' }}>
              {items.map(({ assignment: a, ex }) => {
                ...
                return (
                  <Link key={a.id} to={`/exercises/${a.exerciseId}`} style={{ textDecoration: 'none' }}>
                    <div style={{
                    ...
                    >
                      {/* image area (gradient + emoji placeholder) */}
                      <div style={{ height: '92px', background: `linear-gradient(135deg, ${r.grad[0]}, ${r.grad[1]})`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                      ...
                      <div style={{ padding: '12px 14px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '8px', fontSize: '0.72rem', fontWeight: 700, color: 'var(--color-accent)' }}>
                          {isDone ? 'Do again' : 'Start'} <ChevronRight size={13} />
                        </div>
                      </div>
                    </div>
                  </Link>
